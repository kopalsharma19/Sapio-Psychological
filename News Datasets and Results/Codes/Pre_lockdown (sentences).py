...
pre_lock_sent = pd.read_excel("C:/Users/HITAKSHI SHAH/Desktop/Pre_lockdown (sent).xlsx")

pd.isnull(pre_lock_sent).any()
pre_lock_sent['Score'].unique()
pre_lock_sent['Score'].value_counts()
pre_lock_sent.info()

pre_lock_sent['Sentence'] = pre_lock_sent.apply(lambda row: str(row['Sentence']).lower(), axis=1)
pre_lock_sent.head()
from string import punctuation
def remove_punctuations(string):
    return ''.join(c for c in string if c not in punctuation)

pre_lock_sent['Sentence'] = pre_lock_sent.apply(lambda row: remove_punctuations(row['Sentence']), axis=1)
...
pre_lock_sent['Score'] = pre_lock_sent.apply(lambda row: convert(row['Score']), axis=1)

from sklearn.model_selection import train_test_split

X = pre_lock_sent['Sentence']
y = pre_lock_sent['Score']
