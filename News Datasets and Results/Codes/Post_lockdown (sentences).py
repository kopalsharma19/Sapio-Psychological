...
print(Counter(stripped).most_common(300))

df2= DataFrame(Counter(stripped).most_common(300),columns=['Sntnc','Frequency'])
print(df2)
...
post_lockdown_sent = pd.read_excel("C:/Users/HITAKSHI SHAH/Desktop/Post_lockdown (Sent).xlsx")

pd.isnull(post_lockdown_sent).any()
post_lockdown_sent['Score'].unique()
post_lockdown_sent['Score'].value_counts()
post_lockdown_sent.info()

post_lockdown_sent['Sentence'] = post_lockdown_sent.apply(lambda row: str(row['Sentence']).lower(), axis=1)
post_lockdown_sent.head()
from string import punctuation
def remove_punctuations(string):
    return ''.join(c for c in string if c not in punctuation)

post_lockdown_sent['Sentence'] = post_lockdown_sent.apply(lambda row: remove_punctuations(row['Sentence']), axis=1)
...
post_lockdown_sent['Score'] = post_lockdown_sent.apply(lambda row: convert(row['Score']), axis=1)

from sklearn.model_selection import train_test_split

X = post_lockdown_sent['Sentence']
y = post_lockdown_sent['Score']
